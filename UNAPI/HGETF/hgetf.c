/* HTTP getter 1.2f
   ...
   By Oduvaldo Pavan Junior 07/2019 v1.2f

   ASM.LIB, BASE64.LIB and crt0msx_msxdos_advanced.rel
   are available at www.konamiman.com

   Use supplied code blocks project or build your command line based on it.
   My (Oduvaldo) version uses fusion-c libraries instead of MSXCHAR.LIB

   Comments are welcome: konamiman@konamiman.com (original creator)
                         ducasp@gmail.com (fast version creator)

   Why a F(aster) version?

   Konamiman made a version that is good for all UNAPI adapters. But, it
   will do nothing for 1/60s or 1/50s letting the UNAPI adapter software
   to "breathe". Some adapters, like the one I've worked on using ESP8266
   do not need such thing and it will slow down transfers. So, use /b switch
   that do not allow breathing and test if it is good for your adapter. I've
   the feeling that both DENYONET and OBSONET won't work well as they are based
   on VDP interrupts to process their work.

   Another advantage of /b is that it won't print transfer progress. MSX VDP
   is not really fast writing text to screen and this also will make performance
   suffer. I would rather download a file in half the time than knowing how much
...
const char* strTitle=
    "HTTP file downloader 1.2f\r\n"
    "By Konamiman, 2011 v1.2f Oduvaldo 2019\r\n"
    "\r\n";

const char* strUsage=
    "Usage: hget <file URL>|<local file with the URL>|con [/l:<local file name>]\r\n"
    "  [/n] [/b] [/c] [/v] [/h] [/x:<headers file name>] [/a:<user>:<password>]\r\n"
    "\r\n"
	"/b: No TCPI/IP Wait or progress indication (faster, won't work all cards).\r\n"
...
int ticksWaited;
int sysTimerHold,sysTimerHoldNoBreath;
...
void ProcessOptions()
{
    ...
    for(i=1; i<argumentsCount; i++) {
        if(ArgumentIs(arguments[i], "v")) {
            verboseMode = 1;
            debug("Verbose mode");
        } else if(ArgumentIs(arguments[i], "b")) {
            bBreathe = false;
            debug("No TCPIP WAIT calls");
...
void EnsureThereIsTcpDataAvailable()
{
	...
	{
		sysTimerHoldNoBreath = *SYSTIMER;
		while(remainingInputData == 0) {
			sysTimerHold = *SYSTIMER;
			if (sysTimerHoldNoBreath != sysTimerHold)
			{
				ticksWaited++;
				sysTimerHoldNoBreath = *SYSTIMER;
			}
