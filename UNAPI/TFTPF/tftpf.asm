	;--- TFTPF server/client for the TCP/IP UNAPI 1.1f
	;    By Konamiman, 4/2010
	;
	;--- Changes by Oduvaldo Pavan Junior in version 1.1f
	;    06/2019
	;
	;- Added the possibility to not show transfer progress as it slows
	;  down the performance
	;
	;- Code doesn't deppend on COMPASS specific way of working, it will
	;  compile fine with sjasm
	;
...
WRQ_ERR:	push	af
	push de	;Error when opening or creating?
	call	SEND_DSKERR
	pop	de
	pop af
	call	PRINT_DSKERR
...
EsHexa2:	push	de
Divide:	push	bc
	push hl	;DE/(IX+7)=DE, remainder A 
	ld	a,d
	ld	c,e
	ld	d,0
	ld	e,(ix+7)
...
NOR2FCB:	push	de
	push hl
	push de	;Name zone is filled with spaces
	pop	hl
	inc	de
	ld	a," "
	ld	(hl),a
...
;--- WAIT_TICKB: This version won't call TCPIP_WAIT and won't wait a tick
...
PRINT_PROGRESS:	db	0	;Whether to print transfer progress and also wait one tick change calling TCPIP_WAIT or not
...
PRESENT_S:	db	27,"x5"	;Disables cursor
	db	"TFTP Client/Server for the TCP/IP UNAPI 1.1f",13,10
	db	"By Konamiman, v1.1f By Oduvaldo, 07/19",13,10,10,"$"
...
INFO_S:	db	"Usage:",13,10,10
	...
	db	"  (only if <remote filename> has a valid MSX-DOS path, and only in DOS 2)",13,10
	db	"  S and R prints transfer progress and P and G won't print it.",13,10
	db	"  WHEN transfer progress is not printed, won't call TCPIP_WAIT and",13,10
	db  "  this will improve performance, but won't work on all adapters.",13,10,10
