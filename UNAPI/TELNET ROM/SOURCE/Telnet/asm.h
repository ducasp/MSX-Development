...
extern void AsmCall(uint address, Z80_registers* regs, register_usage inRegistersDetail, register_usage outRegistersDetail) __sdcccall(0);

//* Execute a DOS function call,
//  it is equivalent to invoking address 5 with function number in register C.
extern void DosCall(byte function, Z80_registers* regs, register_usage inRegistersDetail, register_usage outRegistersDetail) __sdcccall(0);

//* Invoke a MSX BIOS routine,
//  this is done via CALSLT to the slot specified in EXPTBL.
extern void BiosCall(uint address, Z80_registers* regs, register_usage outRegistersDetail) __sdcccall(0);


/* --- UNAPI related functions  --- */

// * Get the number of installed implementations of a given specification
extern int UnapiGetCount(char* implIdentifier) __sdcccall(0);
...
extern void UnapiBuildCodeBlock(char* implIdentifier, int implIndex, unapi_code_block* codeBlock) __sdcccall(0);
...
extern void UnapiParseCodeBlock(unapi_code_block* codeBlock, byte* slot, byte* segment, uint* address) __sdcccall(0);
...
extern void UnapiGetRamHelper(uint* jumpTableAddress, uint*  mapperTableAddress) __sdcccall(0);

//* Execute an UNAPI function.
//  Code block can be generated with UnapiBuildCodeBlock.
extern void UnapiCall(unapi_code_block* codeBlock, byte functionNumber, Z80_registers* registers, register_usage inRegistersDetail, register_usage outRegistersDetail) __sdcccall(0);

//* Read one byte from an UNAPI implementation.
//  Code block can be generated with UnapiBuildCodeBlock.
extern byte UnapiRead(unapi_code_block* codeBlock, uint address) __sdcccall(0);
