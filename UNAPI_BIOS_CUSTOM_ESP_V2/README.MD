# Wi-Fi / SSH BIOS for ESP32 and ESP8266 w/ Custom Firmware

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/R6R2BRGX6)

This BIOS will offer the SM-X, MSX PICO, etc., users the following TCP-IP UNAPI functionality:
...
It doesn't implement the following functionality TCP-IP UNAPI functionality:
...
Also, **if it detects an ESP32** it will offer SSH UNAPI functionality, except for
SCP (to be implemented) and SFT (no plans at this moment).

By default, it will automatically install itself and make available an UNAPI
TCP-IP implementation for usage by any applications. If an ESP32 is detected
it will make available an UNAPI SSH implementation. Also, by default, the
...
otherwise TLS/SSL connections will fail. ESP32 has no such restrictions.

If you were using a RAM Driver, please remove RAMHELPR.COM, ESP8266.COM, WAITWIFI.COM,
...
Quick Receive needs support in the interface between ESP and z80, that
...
This BIOS is a simple tunnel for a Custom ESP firmware that implements
...
I've made customized ESP8266 and ESP32 firmwares that are distributed for free
for SM-X / MSX-SM owners, but, if you want to use it on your own device, contact
me so we can talk about it, commercial usage should be fine but I'd like to know
where and how it is being used (and perhaps help if I have some time).

This BIOS assumes the use of an ESP with a custom firmware in a MSX-SM serial
interface OR the equivalent on Memory Mapped IO (for MSX PICO and other devices).
That is the interface specification:
...
For Memory Mapped consider the same description of IO 0x06 should be implemented
on 0x7F06, for IO 0x07 on 0x7F07 and if implementing F2 device for faster boot,
for IO 0xF2 on 0x7F05 (there is a specific readme file for F2 device)
...
(c)2020 - 2026 Oduvaldo Pavan Junior - ducasp@gmail.com
