; ESPUNAPI TCP/IP SSH UNAPI BIOS v.1.0
; Oduvaldo Pavan Junior
; ducasp@gmail.com
;    This code implements TCP/IP and SSH UNAPI SpecificationM
;
...
;
; This code has contribution from Jeroen Taverne that helped making some
; MACROs on the fixed / non speed optimized loop I/O parts of the code so
; we can easily switch this from I/O based to Memory Based.
;
; Note: this implementation depends upon ESP having the UNAPI
...
; EF - Set when ESP was not found or by user at runtime to disable UNAPI
;      next warm boot
;
...
IS_SCREEN1:
    ...
    READ_F2                         ; a = #FF, default value if it comes from power off or hard reset
    cpl                             ; a = #FF - a
    ld  c,a                         ; c = #00
    WRITE_F2                        ; a = #00
    ...
    WRITE_F2                        ; If yes, UNAPI driver is not loaded and exits, but need to restore F2 port to not have a false error
...
INIT_UNAPI:
    ...
    SEND_DATA                       ; Just send, no need to wait response
    ret
INIT_UNAPI2:
...
DO_HTIMI:
    ...
    ld  a,l
    or  h                           ; In this operation, check if HL is 0
...
DO_EXTBIO3SSH:                         ; A=A-1 already done
    ex  (sp),hl
    ld  de,#2222
    ret
...
FN_TABLE_S:
FN_0_S:                 dw  UNAPI_GET_INFO_SSH
FN_1_S:                 dw  SSH_GET_CAPAB
FN_2_S:                 dw  SSH_OPEN
FN_3_S:                 dw  SSH_CLOSE
FN_4_S:                 dw  SSH_STATE
FN_5_S:                 dw  SSH_SEND
...
;===  UNAPI_GET_INFO  ===
;========================
; Obtain the implementation name and version for TCP/IP.
;
;    Input:  A  = 0
...
;===  UNAPI_GET_INFO  ===
;========================
; Obtain the implementation name and version for SSH.
;
;    Input:  A  = 0
;    Output: HL = Descriptive string for this implementation, on this slot, zero terminated
;            DE = API version supported, D.E
...
;===   SSH_GET_CAPAB   ===
;=========================
; Get information about the SSH capabilities and features.
;
;    Input:  A = 1
;            B = Index of information block to retrieve:
;                1: Block 1 (only block supported, other blocks reserved for expansion)
;    Output: A = Error code
...
SSH_GET_CAPAB_ST1:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_ST1
SSH_GET_CAPAB_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #81                         ; Is response of our command?
    jr  nz,SSH_GET_CAPAB_ST1
    ; Now get return code, if return code other than 0, it is finished
SSH_GET_CAPAB_RC:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_RC.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_RC
SSH_GET_CAPAB_RC.1:
    ; nz, discard
...
SSH_GET_CAPAB_ST2:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_ST2.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_ST2
SSH_GET_CAPAB_ST2.1:
    ; Nz, discard
    RECEIVE_DATA
    dec b
    jr  nz,SSH_GET_CAPAB_ST2

SSH_GET_CAPAB_BLK1_ST1:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_BLK1_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_BLK1_ST1
SSH_GET_CAPAB_BLK1_ST1.1:
    ; nz, get it
    RECEIVE_DATA
    ld  l,a
SSH_GET_CAPAB_BLK1_ST2:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_BLK1_ST2.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_BLK1_ST2
SSH_GET_CAPAB_BLK1_ST2.1:
    ; nz, get it
    RECEIVE_DATA
    ld  h,a
SSH_GET_CAPAB_BLK1_ST3:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_BLK1_ST3.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_BLK1_ST3
SSH_GET_CAPAB_BLK1_ST3.1:
    ; nz, get it
    RECEIVE_DATA
    ld  c,a
SSH_GET_CAPAB_BLK1_ST4:
    CHECK_DATA
    jr  nz,SSH_GET_CAPAB_BLK1_ST4.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_GET_CAPAB_BLK1_ST4
SSH_GET_CAPAB_BLK1_ST4.1:
    ; nz, get it
    RECEIVE_DATA
    ld  b,a
...
TCPIP_UDP_OPEN:
    ...
    SEND_DATA                       ; Send the command size lsb
    ld  a,l
    SEND_DATA                       ; Send the port LSB
    ld  a,h
    SEND_DATA                       ; Send the port MSB
    ld  a,b
    SEND_DATA                       ; Send the connection transient/resident
...
TCPIP_UDP_SEND:
    ...
    pop hl
    ld  a,(de)
    SEND_DATA                       ; Send IP byte 1
    inc de
    ld  a,(de)
    SEND_DATA                       ; Send IP byte 2
    inc de
    ld  a,(de)
    SEND_DATA                       ; Send IP byte 3
    inc de
    ld  a,(de)
    SEND_DATA                       ; Send IP byte 4
    inc de
    ld  a,(de)
    SEND_DATA                       ; Send Port LSB
    inc de
    ld  a,(de)
    SEND_DATA                       ; Send Port MSB
...
TCPIP_TCP_OPEN_HOSTNAME_SENDCMDWITHHOSTNAME:
    ...
    ld  b,11                        ; Send 11 bytes
TCPIP_TCP_OPEN_HOSTNAME_SENDCMDWITHHOSTNAME_LOOP_PARAM:
    ld  a,(hl)
    ld  (MEM_OUT_TX_PORT),a
    inc hl
    djnz TCPIP_TCP_OPEN_HOSTNAME_SENDCMDWITHHOSTNAME_LOOP_PARAM
    endif
    ex  de,hl                       ; Size in HL
    ld  de,11
    or  a                           ; zero flag
...
TCPIP_TCP_OPEN_SENDHOSTNAME:
    ...
    ld  (MEM_OUT_TX_PORT),a
    inc hl
    djnz TCPIP_TCP_OPEN_SENDHOSTNAME
    endif
    dec d
    jr  nz,TCPIP_TCP_OPEN_SENDHOSTNAME
...
SSH_OPEN_1:
    ; let's check how many bytes there are
    ld  de,8                        ; Start with 8, all params except username and password
SSH_OPEN_CHECK_USER:
    ld  a,(hl)
    inc de                          ; Increase size count
    inc hl                          ; And next byte
    or  a                           ; If zero, user terminated
    ; Loop until terminator (0) is found
    jp  nz,SSH_OPEN_CHECK_USER
SSH_OPEN_CHECK_PASS:
    ld  a,(hl)
    inc de                          ; Increase size count
    inc hl                          ; And next byte
    or  a                           ; If zero, password terminated
    ; Loop until terminator (0) is found
    jp  nz,SSH_OPEN_CHECK_PASS

SSH_OPEN_SENDCMD:
    ; Ok, so, DE has the full data size, let's start sending from here
    ld  a,#82                       ; Function SSH OPEN
    SEND_DATA                       ; Send the command
    ld  a,d
    SEND_DATA                       ; Send the command size msb
    ld  a,e
...
SSH_OPEN_SENDDATA:
    outi
    jr  nz,SSH_OPEN_SENDDATA
    else
SSH_OPEN_SENDDATA:
    ld  a,(hl)
    ld  (MEM_OUT_TX_PORT),a
    inc hl
    djnz SSH_OPEN_SENDDATA
    endif
    dec d
    jr  nz,SSH_OPEN_SENDDATA
    ; Now wait up to 3600 (1 minute @ 60Hz) ticks to get response
...
SSH_OPEN_ST1:
    CHECK_DATA
    jr  nz,SSH_OPEN_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_OPEN_ST1
SSH_OPEN_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #82                         ; Is response of our command?
    jr  nz,SSH_OPEN_ST1
    ; now get return code, if return code other than 0, it is finished
SSH_OPEN_RC:
    CHECK_DATA
...
SSH_OPEN_CONN_ST1:
    CHECK_DATA
    jr  nz,SSH_OPEN_CONN_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_OPEN_CONN_ST1
SSH_OPEN_CONN_ST1.1:
    ; nz, get it
    RECEIVE_DATA
    ld  b,a
...
SSH_CLOSE_ST1:
    CHECK_DATA
    jr  nz,SSH_CLOSE_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_CLOSE_ST1
SSH_CLOSE_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #83                         ; Is response of our command?
    jr  nz,SSH_CLOSE_ST1
    ; now get return code, if return code other than 0, it is finished
SSH_CLOSE_RC:
    CHECK_DATA
    jr  nz,SSH_CLOSE_RC.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_CLOSE_RC
SSH_CLOSE_RC.1:
    ; nz, discard
...
;===     SSH_STATE     ===
;=========================
;Get the state of a SSH connection.
;
;Input:  A = 4
;        B = Connection number
;Output: A = Error code
...
SSH_STATE_ST1:
    CHECK_DATA
    jr  nz,SSH_STATE_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_STATE_ST1
SSH_STATE_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #84                         ; Is response of our command?
    jr  nz,SSH_STATE_ST1
    ; now get return code, if return code other than 0, it is finished
SSH_STATE_RC:
    CHECK_DATA
    jr  nz,SSH_STATE_RC.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_STATE_RC
SSH_STATE_RC.1:
    ; nz, discard
    ...
    ; next two bytes are size bytes, don't care, it is fixed as 3 if ok
    ld  b,2
SSH_STATE_ST2:
    CHECK_DATA
    jr  nz,SSH_STATE_ST2.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_STATE_ST2
SSH_STATE_ST2.1:
    ; nz, discard
    ...
    ; now we have three bytes to get, first go to B and second to L and third to H
SSH_STATE_REG_B:
    CHECK_DATA
    jr  nz,SSH_STATE_REG_B.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_STATE_REG_B
SSH_STATE_REG_B.1:
    ; read and move it to B
    RECEIVE_DATA
    ld  b,a

SSH_STATE_REG_L:
    CHECK_DATA
    jr  nz,SSH_STATE_REG_L.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_STATE_REG_L
SSH_STATE_REG_L.1:
    ; read and move it to L
    RECEIVE_DATA
    ld  l,a

SSH_STATE_REG_H:
    CHECK_DATA
    jr  nz,SSH_STATE_REG_H.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_STATE_REG_H
SSH_STATE_REG_H.1:
    ; read and move it to H
    RECEIVE_DATA
    ld  h,a
...
;===   SSH_TERM_TYPE   ===
;=========================
;Get or set the terminal type requested on PTY connection.
;
;Input:  A = 7
;        C = Operation ( 0 - Get / 1 - Set)
;        D = Terminal Type when Set
;Output: A = Error code
;        D = Terminal Type (when Get)
SSH_TERM_TYPE:
    set 7,a                         ; ESP SSH commands start at #80
    SEND_DATA                       ; Send the command
    xor a
    SEND_DATA                       ; Send the command size msb
    ld  a,c
    call    SETBYTE                 ; Save operation
    or  a
    jr  nz,SSH_TERM_TYPE_SET
    ld  a,1                         ; Get sends 1 byte
    SEND_DATA                       ; Send the command size lsb
    ld  a,c
    SEND_DATA                       ; Send the operation
    jr  SSH_TERM_TYPE_ST0
SSH_TERM_TYPE_SET:
    ld  a,2                         ; Set sends 2 bytes
    SEND_DATA                       ; Send the command size lsb
    ld  a,c
    SEND_DATA                       ; Send the operation
    ld  a,d
    SEND_DATA                       ; Send the Terminal Type to set
SSH_TERM_TYPE_ST0:
    ; Now wait up to 180 ticks to get response
    ld  hl,180
    call    SETCOUNTER
SSH_TERM_TYPE_ST1:
    CHECK_DATA
    jr  nz,SSH_TERM_TYPE_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_TERM_TYPE_ST1
SSH_TERM_TYPE_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #87                         ; Is response of our command?
    jr  nz,SSH_TERM_TYPE_ST1
    ; now get return code, if return code other than 0, it is finished
SSH_TERM_TYPE_RC:
    CHECK_DATA
    jr  nz,SSH_TERM_TYPE_RC.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_TERM_TYPE_RC
SSH_TERM_TYPE_RC.1:
    ; nz, discard
    ...
    or  a                           ; 0?
    ret nz                          ; if not, done
    call    GETBYTE                 ; restore operation
    or  a
    ret nz                          ; if set all done
    ; next two bytes are return code and size bytes, don't care, it is 0
    ld  b,2
SSH_TERM_TYPE_ST2:
    CHECK_DATA
    jr  nz,SSH_TERM_TYPE_ST2.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_TERM_TYPE_ST2
SSH_TERM_TYPE_ST2.1:
    ; nz, discard
    RECEIVE_DATA
    dec b
    jr  nz,SSH_TERM_TYPE_ST2
    ; next byte is the terminal type
SSH_TERM_TYPE_REGD:
    CHECK_DATA
    jr  nz,SSH_TERM_TYPE_REGD.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_TERM_TYPE_REGD
SSH_TERM_TYPE_REGD.1:
    RECEIVE_DATA
    ld  d,a
    ; done, no return data other than return code
    xor a
...
;===    SSH_WIN_SIZE   ===
;=========================
;Get or set the terminal window size requested on PTY connection.
;
;Input:  A = 8
;        C = Operation ( 0 - Get / 1 - Set)
;        H = Rows when Set
...
SSH_WIN_SIZE:
    ...
    jr  SSH_WIN_SIZE_ST0
SSH_WIN_SIZE_SET:
    ld  a,3                         ; Set sends 3 bytes
    SEND_DATA                       ; Send the command size lsb
    ld  a,c
    SEND_DATA                       ; Send the operation
    ld  a,h
    SEND_DATA                       ; Send the Rows
    ld  a,l
    SEND_DATA                       ; Send the Columns
SSH_WIN_SIZE_ST0:
    ; Now wait up to 180 ticks to get response
    ld  hl,180
    call    SETCOUNTER
SSH_WIN_SIZE_ST1:
    CHECK_DATA
    jr  nz,SSH_WIN_SIZE_ST1.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_WIN_SIZE_ST1
SSH_WIN_SIZE_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #88                         ; Is response of our command?
    jr  nz,SSH_WIN_SIZE_ST1
    ; now get return code, if return code other than 0, it is finished
SSH_WIN_SIZE_RC:
    CHECK_DATA
    jr  nz,SSH_WIN_SIZE_RC.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_WIN_SIZE_RC
SSH_WIN_SIZE_RC.1:
    ; nz, discard
...
SSH_WIN_SIZE_ST2.1:
    ...
    RECEIVE_DATA
    dec b
    jr  nz,SSH_WIN_SIZE_ST2
    ; next byte is Rows
SSH_WIN_SIZE_REGH:
    CHECK_DATA
    jr  nz,SSH_WIN_SIZE_REGH.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_WIN_SIZE_REGH
SSH_WIN_SIZE_REGH.1:
    RECEIVE_DATA
    ld  h,a
    ; next byte is Columns
SSH_WIN_SIZE_REGL:
    CHECK_DATA
    jr  nz,SSH_WIN_SIZE_REGL.1
    call    TCPIP_GENERIC_CHECK_TIME_OUT
    jr  SSH_WIN_SIZE_REGL
SSH_WIN_SIZE_REGL.1:
    RECEIVE_DATA
    ld  l,a
    ; done, no return data other than return code
...
TCPIP_TCP_SEND:
    ...
    SEND_DATA                       ; Send the connection #
    ld  a,c
    SEND_DATA                       ; Send the connection flags
    pop hl
    pop de
...
SSH_SEND_R.1:
    ; nz, check the data
    RECEIVE_DATA
    cp  #85                         ; Is response of our command?
    jr  nz,SSH_SEND_ST1
    ; now get return code, if return code other than 0, it is finished
SSH_SEND_RC:
    CHECK_DATA
...
;===     SSH_RCV     ===
;=======================
;Receive data from a SSH connection.
;
;Input:   A  = 6
;         B  = Connection number
;         DE = Address for the retrieved data
...
SSH_RCV_RETRY_QRCV:
    call    GETBYTE
    or  a
    jr  z,SSH_RCV_CHECK_TIME_OUT.NORXRETRY
    ; Ok, so let's ask ESP to re-send the data and retry receiving it
    dec a
    call    SETBYTE                 ; we are retrying it
    ld  a,'r'                       ; retry transmission command
    SEND_DATA
    jp  SSH_RCV.RXRETRY             ; and retry it
SSH_RCV_CHECK_TIME_OUT.NORXRETRY:
    ld  a,ERR_INV_OPER
    ret                             ; and return the function itself
...
SSH_RCV_ST1.1:
    ; nz, check the data
    RECEIVE_DATA
    cp #86                          ; Is response of our command?
    jr  nz,SSH_RCV_ST1
    ; At this point, all data is being buffered, so 15 ticks, quarter second, is more than enough time-out
    di
    ld  hl,30
...
SSH_RCV_ST2B.1:
    ...
    ld  c,IN_DATA_PORT
    CHECK_QUICK_RECEIVE
    jr  z,SSH_RCV_R_NSF             ; If not, go to the old, slower route
    ; Otherwise, let's speed it up baby!
SSH_RCV_R:
    inir
    else
...
SSH_RCV_R:
    ...
    jr nz,SSH_RCV_R
    CHECK_BUFFER_UNDERRUN
    jp  nz,SSH_RCV_RETRY_QRCV ; If yes, retry
    ; Otherwise, done
    call    BCRESTORE               ; done, restore return data in BC
    ; no urgent data support
    ld  hl,0
...
TCPIP_CONFIG_AUTOIP:
    ...
    SEND_DATA                       ; Send the command
    ld  a,c
    SEND_DATA                       ; Send the command parameter
...
TCPIP_CONFIG_IP:
    ...
    SEND_DATA                       ; Send the command size lsb
    ld  a,b
    SEND_DATA                       ; Send the address to set
    ld  a,l
    SEND_DATA                       ; Send the IP first byte
    ld  a,h
    SEND_DATA                       ; Send the IP second byte
    ld  a,e
    SEND_DATA                       ; Send the IP third byte
    ld  a,d
    SEND_DATA                       ; Send the IP fourth byte
...
;--- Return the saved ESP TYPE
;    Input:  none
;    Output: A and F will be NZ if ESP32
;    Modifies: AF
...
WELCOME:
    db  CLS
    db  "ESP TCP/IP UNAPI 1.0"              ,CR,LF
    db  1,92," 2026 Oduvaldo Pavan Junior"  ,CR,LF
    db  "ducasp@gmail.com"                  ,CR,LF,LF,STTERMINATOR
;---

WELCOME32:
    db  CLS
    db  "ESP TCP/IP SSH UNAPI 1.0"          ,CR,LF
    db  1,92," 2026 Oduvaldo Pavan Junior"  ,CR,LF
    db  "ducasp@gmail.com"                  ,CR,LF,LF,STTERMINATOR
;---
...
FAIL_F: 
    db  CLS 
    db  "ESP FW Update Required!"           ,CR,LF,STTERMINATOR
...
APIINFO:                db  "ESP TCP-IP UNAPI",0
APIINFO_SSH:            db  "ESP SSH UNAPI",0

    if  TURBO_R_LOGO = 0
ID_END: ds  #7FE0-ID_END,#C9
;--- FS-A1GT compliant BIOS without logo (not for FS-A1ST BIOS)
BUILD_NAME:             db  "[ ESPUNAPI.ROM ]"
    else
