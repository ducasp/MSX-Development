...
V9938_PrintChar:
	...
	LD	A,(#HMMC_BUF_COUNT)
	OR	A
	JP	NZ,V9938_PrintChar.NotFirst
	; It is the first, so we need to save the current DXL/DXH
	LD	HL,(#HMMCCURR.DXL)
	LD	(#HMMC_CMD.DXL),HL
V9938_PrintChar.NotFirst:
	LD	E,A
	LD	D,#0
	LD	HL,#HMMC_BUF
	...
	LD	A,(#Concealed)
	OR	A
	JR	NZ,V9938_PrintCharB.SPC	; Concealed = not visible -> just print space
	LD	A,B						; Restore char to print in A
	CP	#0x20
	JR	Z,V9938_PrintCharB.SPC	; Space -> just blank / background color
	CP	#0xDB
	JR	Z,V9938_PrintCharB.FIL	; Fill -> just filled / foreground color	
...
V9938_PrintChar.RowLoop:		
	...
	ADD	A,E
	LD	E,A						; Ok E has it
	JR	NC,V9938_PrintChar.NC	; If no carry no need to INC D
	INC	D
V9938_PrintChar.NC:	
	DEC	IXH						; Decrement counter
	JP	NZ,V9938_PrintChar.RowLoop	
V9938_PrintChar.Done:
	POP IX
	; If here, done
	LD	A,(#HMMC_BUF_COUNT)
	...
	RET							; And done!
V9938_PrintCharB.SPC:	
	LD	A,(#ColorTable+0)		
	LD	B,#8					; 8 lines
	LD	DE,#28					; Our line jump
V9938_PrintCharB.SPL:	
	...
	DJNZ	V9938_PrintCharB.SPL; Decrement counter, if not zero, do it again
	JP	V9938_PrintChar.Done	; Done
V9938_PrintCharB.FIL:	
	LD	A,(#ColorTable+3)		
	LD	B,#8					; 8 lines
	LD	DE,#28					; Our line jump
	JP	V9938_PrintCharB.SPL
...
DO_HMMC_FLUSH:
	...
	ADD	A,A
	LD	(#HMMC_CMD.NXL),A		; And lenght is basically this number * 2
	LD	A,#30					; a line is up to 30 byte long
	SUB	A,E						; we decrease how many bytes we are going to send per line in fact
	LD	(#HMMC_CMD.JMP),A		; this is our jump vector every loop finished
	...
	JP	DO_HMMC_FLUSH.LOOP1ST	; First one is just two outi
DO_HMMC_FLUSH.LOOP:	
	OUTI						; Send line data
DO_HMMC_FLUSH.LOOP1ST:		
	OUTI						; Send line data
	OUTI						; Send line data
	JP	NZ,DO_HMMC_FLUSH.LOOP	; If not done send three more
	DEC	D						; Decrement our line counter
	JP	Z,DO_HMMC_FLUSH.DONE	; if finished, terminate
	LD	B,E						; Restore B for next OUTI
	LD	A,(#HMMC_CMD.JMP)		; How much we need to jump
	ADD	A,L						; Add
	LD	L,A						; Put in L
	JP	NC,DO_HMMC_FLUSH.LOOP	; If no carry done
	INC	H						; If carry, increment H
	JP	DO_HMMC_FLUSH.LOOP		; Back to loop
DO_HMMC_FLUSH.DONE:				
	XOR	A
	LD	(#HMMC_BUF_COUNT),A		; all has ben sent	
	LD	(#HMMC_CMD.NXL),A		; And lenght is now 0
...
HMMC_CMD.JMP:	.db	#00
HMMCCURR.DXL:	.db	#0x10
HMMCCURR.DXH:	.db	#0x00
