; MSX2ANSI ANSI V9938 Library v.1.7 for ROM
;
...
;
; v1.7 for ROM:
; OPJ - Move library ram data to a fixed address 0xC002 so it works for ROM
; as long as you reserve about 300 bytes starting at 0xC002
; OPJ - Changed the way to handle Cursor Position Call Back, this is needed
; because the code was self modifying itself putting the address of the call
; back function and now code is on ROM, so now code calls to a RAM portion,
; that RAM portion will have a call to the call back and then a return so the
; address can change since it is in RAM area now
;
; v1.7:
...
_AnsiInit::
	...
	PUSH	IY
    ; Move variables to heap reserved space at 0xC002
    LD      BC,#Data.End - #OrgAddress
    LD      DE,#0XC002
    LD      HL,#OrgAddress
    LDIR
...
_AnsiStartBuffer::
StartBuffer:
	LD	A,(#0XC002 + #CursorOn - #OrgAddress)
	OR	A
	RET	Z
	CALL	DisCursorSub
	XOR	A
	LD (#0XC002 + #CursorUpdt - #OrgAddress),A
	RET
...
_AnsiCallBack::
	LD	(#0XC002 + #RAM_ANSI_CGP.CALL_VEC - #OrgAddress + 1),HL	; Load the callback function address
	LD	A,#0x01					
	LD	(#0XC002 + #ANSI_CB - #OrgAddress),A			; Flag that have a callback function
	RET
...
_AnsiGetCursorPosition::
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)			; Get Current Cursor Column Position
	INC	A						; Increment it (internally it is 0-79)
	LD	L,A						; Place column in L
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Get Current Cursor Row (line) Position
...
BufferChar.NotCCode:	
	LD	(#0XC002 + #LastChar - #OrgAddress),A
	CALL	V9938_PrintChar		; Call the print routine for our chip
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)			; Get Current Cursor Position
	INC	A						; Increment it
	LD	(#0XC002 + #CursorCol - #OrgAddress),A			; Save
...
BufferChar.LF:
	XOR	A
	LD	(#0XC002 + #CursorCol - #OrgAddress),A			; Save new cursor position
...
BufferChar.CCode:	
	...
	; It is ESC
BufferChar.ESC:	
	LD	(#0XC002 + #ANSI_M - #OrgAddress),A				; Indicate ESC is in progress
	LD	HL,#0XC002 + #ANSI_S - #OrgAddress
	LD	(HL),A					; Save in first buffer position
	INC	HL						; Next buffer position
	LD	(#0XC002 + #ANSI_P - #OrgAddress),HL			; Save in the pointer 
	RET							; Done for now
BufferChar.CNT:	
	LD	HL,(#0XC002 + #ANSI_P - #OrgAddress)			; Current buffer free position
	...
	INC	HL
	LD	(#0XC002 + #ANSI_P - #OrgAddress),HL			; new buffer position
	CP	#48
...
BufferChar.END:	
	XOR	A
	LD	(HL),A
	LD	(#0XC002 + #ANSI_M - #OrgAddress),A				; No longer processing
BufferChar.RET:	
	LD	HL,#0XC002 + #ANSI_S - #OrgAddress
...
BufferChar.CH2a:	
	LD	(#0XC002 + #ANSI_M - #OrgAddress),A				; Ok, now we are gathering parameters for the command
	LD	(#0XC002 + #ANSI_P - #OrgAddress),HL			; Save pointer
	RET							; Done
...
PrintText.RLP.LFeed:
	XOR	A
	LD	(#0XC002 + #CursorCol - #OrgAddress),A
	JP	LineFeed
PrintText.RLP.CC:	
	; Check if it is a control character and do the action, otherwise print it
	OR	A
...
Parameters:
	LD	(#0XC002 + #OrgAddress - #OrgAddress),HL
	LD	DE,#0XC002 + #Parameters.PRM - #OrgAddress
	LD	(#0XC002 + #Parameters.PPT - #OrgAddress),DE
	XOR	A
	LD	(#0XC002 + #Parameters.PCT - #OrgAddress),A
Parameters.RLP:	
	LD	DE,#0XC002 + #Parameters.PST - #OrgAddress		; PARAMETER STRING
	LD	C,#0
Parameters.SCN:	
	LD	A,(HL)
	INC	HL
	...
	JR	Parameters.SCN
Parameters.END:	
	LD	(#0XC002 + #Parameters.TRM - #OrgAddress),A		; SAVE TERMINATING CHAR
	LD	A,C
	OR	A
	...
	LD	(#0XC002 + #Parameters.TRM - #OrgAddress),A		; TERMINATING CHARACTER
	POP	AF
	JR	Parameters.SET
Parameters.ERR:
	LD	HL,(#0XC002 + #EndAddress - #OrgAddress)		; Do not print sequences we do not handle
	XOR	A
	JP	PrintText.RLP
Parameters.RD1:	
	LD	A,(#0XC002 + #Parameters.PST - #OrgAddress)
	SUB	#48
	JR	Parameters.SET
Parameters.RD2:	
	LD	A,(#0XC002 + #Parameters.PST - #OrgAddress) ; was without #
	SUB	#48
	ADD	A,A
	LD	C,A
	ADD	A,A
	...
	ADD	A,C
	LD	C,A
	LD	A,(#0XC002 + #Parameters.PST - #OrgAddress +1)
	SUB	#48
	ADD	A,C
	JR	Parameters.SET
Parameters.RD3:	
	...
	ADD	A,C
	LD	C,A
	LD	A,(#0XC002 + #Parameters.PST - #OrgAddress+2)
	SUB	#48
	ADD	A,C
Parameters.SET:
	LD	DE,(#0XC002 + #Parameters.PPT - #OrgAddress)	; PARAMETER POINTER
	LD	(DE),A
	INC	DE
	LD	(#0XC002 + #Parameters.PPT - #OrgAddress),DE
	LD	A,(#0XC002 + #Parameters.PCT - #OrgAddress)		; PARAMETER COUNT
	INC	A
Parameters.SETOMT:
	LD	B,A
	LD	(#0XC002 + #Parameters.PCT - #OrgAddress),A
	LD	A,(#0XC002 + #Parameters.TRM - #OrgAddress)		; TERMINATING CHARACTER
	CP	#';'
	JP	Z,Parameters.RLP
	LD	(#0XC002 + #EndAddress - #OrgAddress),HL
	CP	#20
	JP	C,Parameters.ERR
	CP	#'H'
	JP	Z,ANSI_CUP
...
ANSI_CGP:						; ANSI Cursor Get Position
	...
	LD	A,(#0XC002 + #Parameters.PRM - #OrgAddress)
	CP	#6						; Is it 6?
	JR	NZ,ANSI_CGP.END			; Not 6, so it is not 6N
	; Ok, ESC[6n, do we have a callback to report cursor position?
	LD	A,(#0XC002 + #ANSI_CB - #OrgAddress)			; Is there a callback programmed?
	OR	A
	JR	Z,ANSI_CGP.END			; Nope, sorry, nothing to do
	; So, lets report the position
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)	
	INC	A
	LD	L,A						; Column goes in L
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)
	INC	A
	LD	H,A						; Row goes in H
ANSI_CGP.CALL_VEC:
	CALL	#0XC002 + #RAM_ANSI_CGP.CALL_VEC - #OrgAddress
ANSI_CGP.END:	
	LD	HL,(#0XC002 + #EndAddress - #OrgAddress)
	JP	PrintText.RLP
...
ANSI_CUP:						; ANSI Cursor Position
	...
	DEC	A
	JR	Z,ANSI_CUP.ROW
	LD	A,(#0XC002 + #Parameters.PRM - #OrgAddress+1)
	DEC	A
	LD	(#0XC002 + #CursorCol - #OrgAddress),A
ANSI_CUP.ROW:	
	LD	A,(#0XC002 + #Parameters.PRM - #OrgAddress+0)
ANSI_CUP.ROW1:	
	DEC	A
	LD	(#0XC002 + #CursorRow - #OrgAddress),A
	JR	ANSI_CUP.RET
ANSI_CUP.HOM:	
	XOR	A
	LD	(#0XC002 + #CursorRow - #OrgAddress),A
...
ANSI_SCP:						; ANSI Save Cursor Position
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)
	LD	(#0XC002 + #SavedCol - #OrgAddress),A
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)
	LD	(#0XC002 + #SavedRow - #OrgAddress),A
	LD	HL,(#0XC002 + #EndAddress - #OrgAddress)
	JP	PrintText.RLP


ANSI_RCP:						; ANSI Restore Cursor Position
	LD	A,(#0XC002 + #SavedCol - #OrgAddress)
	LD	(#0XC002 + #CursorCol - #OrgAddress),A
	LD	A,(#0XC002 + #SavedRow - #OrgAddress)
	LD	(#0XC002 + #CursorRow - #OrgAddress),A
	CALL	V9938_SetCursorX
	CALL	V9938_SetCursorY
	LD	HL,(#0XC002 + #EndAddress - #OrgAddress)
...
ANSI_DCH:						; ANSI Delelete Characters
	...
	JP	V9938_DelChr
ANSI_DCH.GP:	
	LD	A,(#0XC002 + #Parameters.PRM - #OrgAddress+0)	; Load parameter, number of characters to delete
	JP	V9938_DelChr
...
ANSI_REP.RLP:
	LD	A,(#0XC002 + #LastChar - #OrgAddress)
	PUSH	BC
	PUSH	HL			
	CALL	V9938_PrintChar		; Call the print routine for our chip
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)			; Get Current Cursor Position
	...
	XOR	A				
	LD	(#0XC002 + #CursorCol - #OrgAddress),A			; Otherwise cursor is back to position 0
...
ANSI_IL:
	...
	OR	A
	JR	Z,ANSI_IL.RUN
	LD	A,(#0XC002 + #Parameters.PRM - #OrgAddress)		; Read how many Rows
	LD	B,A
ANSI_IL.RUN:
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)
	...
	XOR	A
	LD	(#0XC002 + #CursorCol - #OrgAddress),A			; For now in first column
	CALL	V9938_ErDis0Sub		; Make ED0
	POP	AF						; Restore column
	LD	(#0XC002 + #CursorCol - #OrgAddress),A			; Back to the right column
...
ANSI_IL.CONTINUE:
	...
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; From current cursor row
...
ANSI_ECH:						; ANSI Erase Character
	...
	LD	A,(#0XC002 + #Parameters.PRM - #OrgAddress)		; How many chars to delete in A
	ADD	C						;
...
ANSI_SGR.RES:					; RESET ATTRIBUTES
	...
	XOR	A				
	LD	(#0XC002 + #HiLighted - #OrgAddress),A
	LD	(#0XC002 + #Reversed - #OrgAddress),A
	LD	(#0XC002 + #Concealed - #OrgAddress),A
	; PK: Some softwares expects that 
	;     reset restore the text and
	;     background colors to a sane 
	;     default
	LD	(#0XC002 + #BackColor - #OrgAddress),A
	LD	A,#0x07
	LD	(#0XC002 + #ForeColor - #OrgAddress),A
	JR	ANSI_SGR.CLR
ANSI_SGR.BLD:	
	LD	A,#0x01
	LD	(#0XC002 + #HiLighted - #OrgAddress),A
	JR	ANSI_SGR.CLR
ANSI_SGR.REV:
	LD	A,(#0XC002 + #Reversed - #OrgAddress)
	OR	A
	JR	NZ,ANSI_SGR.CLR
	LD	A,#0x01
	LD	(#0XC002 + #Reversed - #OrgAddress),A
...
ANSI_SGR.CLR:	
	...
	JR	ANSI_SGR.UNK
ANSI_SGR.SWP:
	LD	A,(#0XC002 + #ForeColor - #OrgAddress)
	LD	B,A
	LD	A,(#0XC002 + #BackColor - #OrgAddress)
	LD	(#0XC002 + #ForeColor - #OrgAddress),A
	LD	A,B
	LD	(#0XC002 + #BackColor - #OrgAddress),A
	JR	ANSI_SGR.CLR
...
VT52_ENCURSOR:
	...
	INC	A
	LD	(#0XC002 + #CursorOn - #OrgAddress),A			; Other than 0, on
	LD	(#0XC002 + #CursorUpdt - #OrgAddress),A			; Other than 0, update its position
EnCursorSub:
	DI
...
VT52_DISCURSOR:
	...
	XOR	A
	LD	(#0XC002 + #CursorOn - #OrgAddress),A			; 0, off
	LD	(#0XC002 + #CursorUpdt - #OrgAddress),A			; 0, do not update its position
DisCursorSub:
	DI
	LD	A,(#VDP_08)				; Get a copy of register 8
...
HorizontalTab:
	...
	JP	PrintText.RLP
HTabSub:
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)			; Get the current column
...
V9938_Init:
	...
	LD	HL,#0XC002 + #ANSI_PAL - #OrgAddress			; Address of our palette
...
V9938_InitCursor:
	...
	LD	HL,#0XC002 + #SPRITE_COLORS - #OrgAddress
	LD	BC,#0x2098
V9938_InitCursor.COLRLOOP:
	...
	LD	HL,#0XC002 + #PATTERN_CURSOR - #OrgAddress
	LD	BC,#0x1098
V9938_InitCursor.PATRNLOOP:	
	...
	LD	HL,#0XC002 + #SPRITE_TABLE - #OrgAddress
	LD	BC,#0x0898
V9938_InitCursor.ATTRLOOP:	
	...
	EI
	LD	(#0XC002 + #CursorOn - #OrgAddress),A			; Other than 0, on
	LD	(#0XC002 + #CursorUpdt - #OrgAddress),A			; Other than 0, update it's position
	RET
...
V9938_CursorColor:
	...
	LD	A,#0b01110100			; Write (bit 6), A13/A12 to 1(1F) / A11 to 0 and A10 to 1 and A9-A8 to 0 (4)
	OUT	(#0x99),A				; High Address	
	LD	A,(#0XC002 + #HiLighted - #OrgAddress)
	OR	A
	LD	A,(#0XC002 + #ForeColor - #OrgAddress)
...
V9938_PrintChar:
	LD	B,A
	LD	A,(#0XC002 + #Concealed - #OrgAddress)
	OR	A
	...
	LD	DE,#0XC002 + #ColorTable - #OrgAddress			; Color Table address in DE
	...
	RLCA
	RLCA
	ADD	A,#0XC002 + #ColorTable - #OrgAddress			; Ok, so this is the base of color table (back/back, back/fore, fore/back, fore/fore) depending on each pixel
	LD	E,A						; move it to E
	LD	A,(DE)					; And A has the result of the two pixels
	LD	(#0XC002 + #HMMC_CMD.CLR - #OrgAddress),A		; Move it to HMMC color pair (the first two pixels of HMMC operation)
...
V9938_PrintChar.SPC:	
	LD	A,(#0XC002 + #ColorTable - #OrgAddress+0)		
	LD	(#0XC002 + #HMMC_CMD.CLR - #OrgAddress),A		; Space is background and background WAS without #
	CALL	DO_HMMC				; setup HMMC and do the first two pixels
	LD	A,(#0XC002 + #ColorTable - #OrgAddress+0)		; Again, background on background
V9938_PrintChar.OUT:	
...
V9938_PrintChar.SPL:	
	...
	RET							; Done
V9938_PrintChar.FIL:	
	LD	A,(#0XC002 + #ColorTable - #OrgAddress+3)
	LD	(#0XC002 + #HMMC_CMD.CLR - #OrgAddress),A		; Fill is foreground and foreground		
	CALL	DO_HMMC				; setup HMMC and do the first two pixels
	LD	A,(#0XC002 + #ColorTable - #OrgAddress+3)		; Again foreground and foreground
...
V9938_SetCursorX:
	...
	LD	(#0XC002 + #SPRITEPOS.X - #OrgAddress),A
	ADD	A,A						; HMMC work with real pixel count, not double pixels
	LD	(#0XC002 + #HMMC_CMD.DXL - #OrgAddress),A
	LD	A,#0					; Do not want to change carry
	JR	NC,V9938_SetCursorX.SETDXH
	INC	A						; If carry it is 1
V9938_SetCursorX.SETDXH:	
	LD	(#0XC002 + #HMMC_CMD.DXH - #OrgAddress),A	
	LD	A,(#0XC002 + #CursorUpdt - #OrgAddress)			; If cursor is being updated, update its position in the table
	OR	A
	RET	Z
	...
	OUT	(#0x99),A				; High Address		
	; X Position
	LD	A,(#0XC002 + #SPRITEPOS.X - #OrgAddress)	
	OUT	(#0x98),A				; Set X
	EI
	RET
...
V9938_SetCursorY:
	...
	LD	(#0XC002 + #HMMC_CMD.DYL - #OrgAddress),A
	LD	A,(#0XC002 + #CursorUpdt - #OrgAddress)			; If cursor is being updated, update its position in the table
	OR	A
	RET	Z
	...
	OUT	(#0x99),A				; High Address		
	; Y Position
	LD	A,(#0XC002 + #HMMC_CMD.DYL - #OrgAddress)
	LD	B,A						; Copy IYL to B
	OUT	(#0x98),A				; Set Y
	EI
	RET
...
;
	...
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; Number of lines * 8 = position of the last line
	LD	A,C
	ADD	A,A
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; Will paint a rectangle with C*8 pixels on the Y axys
	XOR	A
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A
	LD	A,#0xE0
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A
	LD	A,#0x01
	LD	(#0XC002 + #HMMV_CMD.NXH - #OrgAddress),A		; The rectangle is 480 pixels on the X axis
	LD	A,B
	OR	A
	LD	A,(#0XC002 + #BackColor - #OrgAddress)
	JR	Z,V9938_ClearBlock.Cont
	LD	A,(#0XC002 + #BorderColor - #OrgAddress)
V9938_ClearBlock.Cont:
	LD	(#0XC002 + #ClearColor - #OrgAddress),A
	ADD	A,A
	ADD	A,A
	ADD	A,A
	ADD	A,A
	LD	B,A
	LD	A,(#0XC002 + #ClearColor - #OrgAddress)
	OR	B						; Adjust color in the right format
	LD	(#0XC002 + #HMMV_CMD.CLR - #OrgAddress),A		; Color to paint the rectangle
	JP	DO_HMMV
...
V9938_ClearTop:
	XOR	A
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; position of the first line
	LD	A,#0x08
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; Will paint a rectangle with 8 pixels on the Y axys
	XOR	A
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A
	LD	A,#0xE0
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A
...
V9938_ClearScreen:
	...
	LD	A,(#0XC002 + #BackColor - #OrgAddress)			; Text1 and Text2 color 15, Border and Background color 0
	LD	(#0XC002 + #BorderColor - #OrgAddress),A			; Save the new border color
	LD	(#VDP_07),A
	OUT	(#0x99),A
	LD	A,#0x80+7
	OUT	(#0x99),A				; Write to register 7
...
V9938_InsertChars:
	...
	LD	(#0XC002 + #HMMM_CMD.NXL - #OrgAddress),A		; Store as NX lower byte
	LD	A,#0x00					; Probably will be 0 NX higher byte
	JR	NC,V9938_InsChr.NXH		; But if carry, means it is 1
	INC	A						; If carry, NXh is 1
V9938_InsChr.NXH:
	LD	(#0XC002 + #HMMM_CMD.NXH - #OrgAddress),A		; Store it
	LD	A,#0x08
	LD	(#0XC002 + #HMMM_CMD.NYL - #OrgAddress),A		; A line has 8 pixels, so NYL is 8
	...
	LD	(#0XC002 + #HMMM_CMD.DXL - #OrgAddress),A		; Now destination is rightmost edge
	LD	A,#0x01					; Rightmost edge in pixels is 0x01DE
	LD	(#0XC002 + #HMMM_CMD.DXH - #OrgAddress),A		; Now destination is rightmost edge
	...
	LD	(#0XC002 + #HMMM_CMD.SXL - #OrgAddress),A		; Save source
	LD	A,#0x00					; XH could be 0
	JR	NC,V9938_InsChr.SXH		; If no carry, it is 0
	INC	A						; Otherwise it is 1
V9938_InsChr.SXH:
	LD	(#0XC002 + #HMMM_CMD.SXH - #OrgAddress),A		; Save XH
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Current cursor line
	ADD	A,A
	ADD	A,A
	...
	LD	(#0XC002 + #HMMM_CMD.DYL - #OrgAddress),A		; This is the Y destination
	LD	(#0XC002 + #HMMM_CMD.SYL - #OrgAddress),A		; As well as the Y source
	LD	A,#0x04					; We need the direction of move to be to the left, otherwise it won't work properly
	LD	(#0XC002 + #HMMM_CMD.ARG - #OrgAddress),A		;
...
V9938_DelChr:
	...
	LD	(#0XC002 + #HMMM_CMD.NXL - #OrgAddress),A		; Store as NX lower byte
	LD	A,#0x00					; Probably will be 0 NX higher byte
	LD	(#0XC002 + #HMMM_CMD.ARG - #OrgAddress),A		; Take this chance to load CMD ARG we need for this operation (00)
	JR	NC,V9938_DelChr.NXH		; But if carry, means it is 1
	INC	A						; If carry, NXh is 1
V9938_DelChr.NXH:
	LD	(#0XC002 + #HMMM_CMD.NXH - #OrgAddress),A		; Store it
	LD	A,#0x08
	LD	(#0XC002 + #HMMM_CMD.NYL - #OrgAddress),A
	; No need to change NYH, always 0
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)
	LD	B,A
	ADD	A,A
	...
	ADD	A,A						; Multiply by 2
	LD	(#0XC002 + #HMMM_CMD.DXL - #OrgAddress),A		; Destination is current cursor position
	LD	A,#0x00					; XH could be 0
	JR	NC,V9938_DelChr.DXH		; If no carry, it is 0
	INC	A						; Otherwise it is 1
V9938_DelChr.DXH:
	LD	(#0XC002 + #HMMM_CMD.DXH - #OrgAddress),A		; Save XH	
	...
	ADD	#0x08					; Add 8 to X (A) - Border of 16 pixels
	ADD	A,A						; Multiply by 2
	LD	(#0XC002 + #HMMM_CMD.SXL - #OrgAddress),A		; Source is current cursor position + deleted characters
	LD	A,#0x00					; XH could be 0
...
V9938_DelChr.SXH:
	...
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)			; Current Cursor Column
...
V9938_DelChr.SL:	LD	HL,(#0XC002 + #EndAddress - #OrgAddress)
	JP	PrintText.RLP	
...
V9938_ErDis0Sub:
	...
	ADD	A,A
	ADD	A,B
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A		; DX = Number of column * 3 (this mode has doubled pixels in X axis)
	LD	B,A
	...
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A		; Store as NX lower byte
	LD	A,#0x00					; Probably will be 0 NX higher byte
	JR	NC,V9938_ErDis0.NXH		; But if carry, means it is 1
	INC	A						; If carry, NXh is 1
V9938_ErDis0.NXH:	
	LD	(#0XC002 + #HMMV_CMD.NXH - #OrgAddress),A		; Store it
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Now get the row / line
	ADD	A,A						; 8 pixels height each character, multiply it per 8
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; This is the Y axys start
	LD	A,#0x08				
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; To clear a single line it is 8 pixels height number of dots height
	LD	A,(#0XC002 + #BackColor - #OrgAddress)
	ADD	A,A
	ADD	A,A
	ADD	A,A
	...
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Let's see how many pixels we need to fill
	...
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; To clear remaining lines it is 8 pixels height multiplied by number of lines
	XOR	A					 	
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A		; DXL 0
	LD	A,#0xE0					; We draw 240 double-pixels (6 pixels wide characters * 80 columns), 480 pixels, 0x01E0
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A		; Store as NX lower byte
	LD	A,#1
	LD	(#0XC002 + #HMMV_CMD.NXH - #OrgAddress),A		; Store NX higher byte
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Now get the row / line
	INC	A						; Next line
	ADD	A,A						; 8 pixels height each character, multiply it per 8
	ADD	A,A
...
V9938_ErDis1:
	XOR	A
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A		; DX = Beginning of line, 0
	LD	(#0XC002 + #HMMV_CMD.DXH - #OrgAddress),A		; DX = Beginning of line, 0
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)
	LD	B,A
	ADD	A,A
...
V9938_ErDis1.NXH:
	...
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; To clear remaining lines it is 8 pixels height multiplied by number of lines - 1 (which is cursor row)
	XOR	A					
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; DYL, DXL ,DXH 0
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A
	LD	(#0XC002 + #HMMV_CMD.DXH - #OrgAddress),A
	LD	A,#0xE0					; We draw 240 double-pixels (6 pixels wide characters * 80 columns), 480 pixels, 0x01E0
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A		; Store as NX lower byte
	LD	A,#1
	LD	(#0XC002 + #HMMV_CMD.NXH - #OrgAddress),A		; Store NX higher byte
	CALL	DO_HMMV				; Aaaand.... Clear!	
V9938_ErDis1.SL:	LD	HL,(#0XC002 + #EndAddress - #OrgAddress)
	JP	PrintText.RLP


V9938_ErChar0:
	LD	A,(#0XC002 + #CursorCol - #OrgAddress)			; Will start from current column
	LD	B,A
	ADD	A,A
	ADD	A,B						; A has column * 3
	LD	(#0XC002 + #HMMV_CMD.DXL - #OrgAddress),A		; Store in the destination X
	LD	A,C						; Characters to delete in C
	ADD	A,A
	ADD	A,C					
	ADD	A,A						; And now double it, chars * 6
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A		; This is the number of pixels to erase
	LD	A,#0x00					; High byte could be 0
	JR	NC,V9938_ErChar0.NXH	; If did not carry, it is
	INC	A						; Otherwise it is 1
V9938_ErChar0.NXH:	
	LD	(#0XC002 + #HMMV_CMD.NXH - #OrgAddress),A		; High Byte number of pixels to erase
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Now calculate destination Y
	ADD	A,A
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; Row * 8 is the Y position
	LD	A,#0x08
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; And delete 8 pixels in Y direction, one line
	LD	A,(#0XC002 + #BackColor - #OrgAddress)
	ADD	A,A
	ADD	A,A
	ADD	A,A
...
V9938_ErLin0.1:
	...
	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A		; And this the X axys lenght of the command
	LD	A,#0x00					; High Byte could be 0
	JR	NC,V9938_ErLin0.NXH		; And it is zero if no carry
	INC	A						; Ok, carry, so it is 1
V9938_ErLin0.NXH:	
	LD	(#0XC002 + #HMMV_CMD.NXH - #OrgAddress),A		; High Byte of X axys lenght
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Now get the current line
	ADD	A,A
	ADD	A,A
	ADD	A,A						; Multiply per 8 as each line is 8 pixelslarge
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; This is the destination Y
	LD	A,#0x08
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; a line is 8 pixes, so this the Y axys lenght of the command
	LD	A,(#0XC002 + #BackColor - #OrgAddress)
	ADD	A,A
	ADD	A,A
	ADD	A,A
...
V9938_ErLin1:
	...
	INC	C
V9938_ErLin1.CNT:	LD	(#0XC002 + #HMMV_CMD.NXL - #OrgAddress),A
	LD	A,C
	JP	V9938_ErLin0.NXH


V9938_ErLin2:
	LD	A,(#0XC002 + #CursorRow - #OrgAddress)			; Clear Entire Line
...
V9938_SetColors:
	LD	A,(#0XC002 + #HiLighted - #OrgAddress)			; Let's check if HiLighted
	OR	A						
	LD	A,(#0XC002 + #ForeColor - #OrgAddress)			; And get the foreground color
...
V9938_SetColors.NHA:	LD	B,A ; Ok, palete color index in B
	...
	LD	(#0XC002 + #FontColor - #OrgAddress),A			; And this is our font color (Fore / Fore)
	LD	A,(#0XC002 + #BackColor - #OrgAddress)			; Do the same to background color
	ADD	A,A
	ADD	A,A
	ADD	A,A
	ADD	A,A						; Multiply by 16 -> Shift left 4 times -> Value in MSB
	LD	B,A						; B has background color in MSB and 0 in LSB
	LD	A,(#0XC002 + #BackColor - #OrgAddress)			; A has background color in LSB
	OR	B						; And value now in LSB as well, colors are pairs for G6
	LD	(#0XC002 + #ColorTable - #OrgAddress+00),A		; ColorTable 0 -> Background and BackGround
	LD	A,(#0XC002 + #FontColor - #OrgAddress)
	AND	#0x0F					; Foreground color on LSB
	OR	B						; Background color on MSB
	LD	(#0XC002 + #ColorTable - #OrgAddress+01),A		; Color table 1 ->Background and Foreground
	LD	A,(#0XC002 + #FontColor - #OrgAddress)
	AND	#0xF0					; Foreground color on MSB
	LD	B,A						; Move it to B
	LD	A,(#0XC002 + #BackColor - #OrgAddress)			; Background color on LSC
	OR	B
	LD	(#0XC002 + #ColorTable - #OrgAddress+02),A		; Color table 2 -> Foreground and Background
	LD	A,(#0XC002 + #FontColor - #OrgAddress)
	LD	(#0XC002 + #ColorTable - #OrgAddress+03),A		; Color table 3 -> Foreground and Foreground
...
V9938_CopyBlockYUpLine:
	...
	ADD	A,A
	ADD	A,A
	LD	(#0XC002 + #YMMM_CMD.NYL - #OrgAddress),A		; Will copy a rectangle with A*8 pixels on the Y axis
	...
	LD	(#0XC002 + #YMMM_CMD.SYL - #OrgAddress),A		; Source Y coordinate
	...
	ADD	A,D						; And add how many lines, as we are going from bottom to top, start at the last line
	LD	(#0XC002 + #YMMM_CMD.DYL - #OrgAddress),A		; To
	...
	LD	A,(#0XC002 + #YMMM_CMD.DYL - #OrgAddress)		; DY
	SUB	A,D						; NY - DY
	; If Carry, this is split case 2,do it
	JR	C,V9938_CopyBlockYUp.DO2
	; Otherwise, it is a single operation so...
	LD	A,#8
	LD	(#0XC002 + #YMMM_CMD.ARG - #OrgAddress),A		; Direction is Up
	LD	(#0XC002 + #YMMM_CMD.DXL - #OrgAddress),A		; And this is our X starting source point as well
...
V9938_CopyBlockYUp.DO1:
	...
	LD	(#0XC002 + #YMMM_CMD.NYL - #OrgAddress),A		; First rectangle of split operation
	LD	A,B						; Restore the overflow # of lines
	LD	(#0XC002 + #YMMM_CMD.NYL2 - #OrgAddress),A		; Second rectangle of split operation
	LD	A,#8					; Direction is Up
	LD	(#0XC002 + #YMMM_CMD.ARG - #OrgAddress),A
	LD	(#0XC002 + #YMMM_CMD.DXL - #OrgAddress),A		; And this is our X starting source point as well
	CALL	DO_YMMM				; Do the first operation
	XOR	A
	LD	(#0XC002 + #YMMM_CMD.SYL - #OrgAddress),A		; Second round Source
	LD	A,(#0XC002 + #YMMM_CMD.NYL - #OrgAddress)		; #of lines of 1st operation in A
	LD	B,A						; Save it in B
	LD	A,(#0XC002 + #YMMM_CMD.DYL - #OrgAddress)		; line of 1st operation destination
	SUB	A,B						; Subtract first SYL to it 
	LD	(#0XC002 + #YMMM_CMD.DYL - #OrgAddress),A		; line of 2nd operation destination
	LD	A,(#0XC002 + #YMMM_CMD.NYL2 - #OrgAddress)		; Second rectangle of split operation in A
	OR	A						; Check if zero
	RET	Z						; If it is, dones
	LD	(#0XC002 + #YMMM_CMD.NYL - #OrgAddress),A		; Save it for next YMMM, so it will be done with DY as 0, SY added w/ # of lines already copied and NYL with the remaining lines to copy
...
V9938_CopyBlockYUp.DO2:
	...
	CALL	DO_YMMM				; Do the first operation
	XOR	A
	LD	(#0XC002 + #YMMM_CMD.DYL - #OrgAddress),A		; Second round To
	LD	A,(#0XC002 + #YMMM_CMD.NYL - #OrgAddress)		; #of lines of 1st operation in A
	LD	B,A						; Save it in B
	LD	A,(#0XC002 + #YMMM_CMD.SYL - #OrgAddress)		; #of starting source lines of 1st operation in A
	SUB	A,B						; Subtract both
	LD	(#0XC002 + #YMMM_CMD.SYL - #OrgAddress),A		; #of starting source lines of 2nd operation
	LD	A,(#0XC002 + #YMMM_CMD.NYL2 - #OrgAddress)		; Second rectangle of split operation in A
	OR	A						; Check if zero
	RET	Z						; If it is, dones
	LD	(#0XC002 + #YMMM_CMD.NYL - #OrgAddress),A		; Save it for next YMMM, so it will be done with DY as 0, SY added w/ # of lines already copied and NYL with the remaining lines to copy
...
V9938_CopyBlockYDownLine:
	...
	; Otherwise, it is a single operation so...
	XOR	A
	LD	(#0XC002 + #YMMM_CMD.ARG - #OrgAddress),A		; Direction is down
	LD	A,#8					; Skip the first 16 pixels as those do not matter (our border)
	LD	(#0XC002 + #YMMM_CMD.DXL - #OrgAddress),A		; And this is our X starting source point
	JP	DO_YMMM					; Do the single operation and YMMM will return for us	
...
DO_HMMC:
	...
	OUT	(#0x99),A
	EI
	LD	HL,#0XC002 + #HMMC_CMD - #OrgAddress			; The HMMC buffer
...
DO_HMMV:
	...
	LD	A,#0x91					; Register #17 (indirect register access auto increment)
	OUT	(#0x99),A
	LD	HL,#0XC002 + #HMMV_CMD - #OrgAddress			; The HMMV buffer
	LD	C,#0x9B					; And port for indirect access
	LD	A,(HL)					; LD DXL in A
	INC	HL
...
DO_HMMV.DXH:	
	...
	LD	A,(#0XC002 + #HMMV_CMD.NYL2 - #OrgAddress)		; It's the second step?
	OR	A
	LD	A,(#VDP_23)				; Get current vertical offset
	JR	Z,DO_HMMV.FIRST
	XOR	A
DO_HMMV.FIRST:
	...
	LD	A,(#0XC002 + #HMMV_CMD.NYL - #OrgAddress)		; NYL
	ADD	A,B						; Ok, now let's check if there is an overflow
	LD	(#0XC002 + #HMMV_CMD.NYL2 - #OrgAddress),A		; Save just in case it is a split operation :D
	...
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; New NYL	
	...
	LD	(#0XC002 + #HMMV_CMD.DYL - #OrgAddress),A		; New DYL
	LD	A,(#0XC002 + #HMMV_CMD.NYL2 - #OrgAddress)		; The remaining lenght at Y
	LD	(#0XC002 + #HMMV_CMD.NYL - #OrgAddress),A		; Now at NYL
...
DO_HMMV.ONESTEP:
	...
	OUTI
	EI
	LD	(#0XC002 + #HMMV_CMD.NYL2 - #OrgAddress),A		; Clear NYL2
	RET
...
DO_HMMM:
	...
	OUT	(#0x99),A
	EI
	LD	HL,#0XC002 + #HMMM_CMD - #OrgAddress			; The HMMC buffer
	LD	C,#0x9B					; And port for indirect access
	CALL	V9938_WaitCmd		; Wait if any command is pending	
	OUTI						; And now send the buffer
	OUTI
...
DO_YMMM:
	...
	OUT	(#0x99),A
	EI	
	LD	HL,#0XC002 + #YMMM_CMD - #OrgAddress			; The YMMM buffer
	LD	C,#0x9B					; And port for indirect access
	CALL	V9938_WaitCmd		; Wait if any command is pending
	OUTI						; And now send the buffer
	OUTI
...
RAM_ANSI_CGP.CALL_VEC:
	.db #0xCD,#0x00,#0x00,#0xC9					        ; This address will change when someone wants to receive callbacks
