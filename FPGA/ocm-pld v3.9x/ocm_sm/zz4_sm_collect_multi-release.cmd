@echo off
rem --- 'zz4_sm_collect_multi-release.cmd' v2.7 by KdL (2021.08.23)
...
if "%DEVICE%"=="smxmini" set DEVSTR= for SM-X Mini&set BIOSLOGO=smx
...
if not exist "%INPDIR%%LOG%" if not exist "%INPDIR%fw\" set FAIL=YES&echo Task canceled "%INPDIR%%PROJECT%.qpf"%DEVSTR%>>"%LOG%"&goto:eof
...
move %INPDIR%fw\recovery.jic %OUTDIR% >nul 2>nul
cd %INPDIR%
